...
匹配的游戏文本会通过标准 `liga` 特性自动转换为彩色符号——例如 Magic 中的 `{W}{U}{B}{R}{G}`，或 Lorcana 中的 `{S}{W}{L}{I}`。
...
- **Sigilora <Game>** —— 全量字体。Magic 另有阴影与扁平样式，用 `font-feature-settings: 'ss01'`（阴影）或 `'ss02'`（扁平）切换。
