...
const game = ref<'magic' | 'lorcana' | 'riftbound' | 'pokemon'>('magic')
...
const defaults: Record<string, string> = {
  ...
  pokemon: 'Attach a Grass {G}, keep Fire [R] for the finish, and save Lightning [L] for retreats.',
}
...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex flex-wrap items-center gap-4">
      <USelect v-model="game" :items="[{ label: $t('game-name.magic'), value: 'magic' }, { label: $t('game-name.lorcana'), value: 'lorcana' }, { label: $t('game-name.riftbound'), value: 'riftbound' }, { label: $t('game-name.pokemon'), value: 'pokemon' }]" class="w-44" @update:model-value="onGameChange" />
    </div>
    ...
    <div class="flex flex-wrap items-center gap-4">
      ...
      <UButtonGroup>
        <UButton
          ...
          :disabled="family === 'lite' || isLorcana || isRiftbound || isPokemon"
          @click="style = 'default'"
        >
          {{ $t('playground.default') }}
        </UButton>
        <UButton :variant="style === 'shadow' ? 'solid' : 'ghost'" :disabled="family === 'lite' || isLorcana || isRiftbound || isPokemon" @click="style = 'shadow'">
          {{ $t('playground.shadow') }}
        </UButton>
        <UButton :variant="style === 'flat' ? 'solid' : 'ghost'" :disabled="family === 'lite' || isLorcana || isRiftbound || isPokemon" @click="style = 'flat'">
          {{ $t('playground.flat') }}
        </UButton>
      </UButtonGroup>
